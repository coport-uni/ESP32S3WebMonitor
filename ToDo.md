...
- [x] 하드웨어 검증 — 부팅 로그로 확인: `parsed 9 entities: 0 lights, 6 switches, 3 sensors` (읽기), 화면 터치 → `/api/services/switch/turn_on ok` + 즉시 재확인 폴 (쓰기). 사용자 확인 "잘 되네". 흰 글씨가 밝은 테마 탭 배경에 묻히는 문제는 검은색으로 교체 후 재플래시하여 해결
- [x] 루트 `README.md` 를 HA 클라이언트 기준으로 재작성 + `LearnedPatterns.md` 에 5건 추가 — §3.13(HA REST 는 id/pw 거부, 장기 토큰 전용), §3.14(HA 는 실기기와 통합 설정 엔티티를 구분할 속성을 노출하지 않음), §3.15(LVGL 9 `lv_obj_add_state` 는 VALUE_CHANGED 를 발송하지 않음 → 가드 불필요), §4.3(펌웨어 작성 전 호스트 스크립트로 서버를 프로브하라), §4.4(권한 분류기의 거부 사유가 대개 더 나은 설계를 알려준다), §5.14(쓴 커넥션으로 검증하지 말 것 — 레지스트리 캐시)
- [x] GitHub Issue #23 생성 → 커밋 `68e5428` 으로 자동 close, origin/main 푸시 완료 (`git push` 가 GCM 프롬프트에서 멈춰 `gh auth setup-git` 으로 해결)
...
메모(보안, 2026-07-16): 커밋 전 자격증명 점검 중 **기존 유출**을 발견 — `examples/hotplate_controller/main/Kconfig.projbuild` 의 `HOTPLATE_WIFI_SSID` / `HOTPLATE_WIFI_PASSWORD` 가 실제 값을 `default` 로 갖고 있었고, 저장소는 PUBLIC 이며 `54a0d12`·`4a2d802` 로 이미 푸시된 상태였다. help 문구의 "Stored in the local sdkconfig only" 는 사용자가 menuconfig 로 입력한 값에만 해당하며 `Kconfig.projbuild` 자체는 추적 파일이라 `default` 는 그대로 커밋된다 — 문구가 방어해주지 않는 것을 방어해준다고 믿게 만들었다. 전수 조사 결과 이 파일 하나뿐이고 나머지 4개 example 과 신규 `main/` 은 전부 `default ""` 로 정상. 68e5428 에서 `default ""` 로 교체 + help 문구 정정했으나 **히스토리에는 남아 있으므로 WiFi 비밀번호 교체가 필요하다(사용자 조치)**. 공개 이슈 등록은 취약점을 저장소 관찰자에게 광고하는 셈이라 임의로 진행하지 않고 사용자 판단에 맡김. 신규 `main/Kconfig.projbuild` 는 WIFI_PASSWORD / TOKEN 모두 `default ""` 이고 실제 값은 gitignore 대상 `sdkconfig` 에만 존재함을 확인.

## 2026-08-20 | Home Assistant 클라이언트를 examples/home_assistant_client 로 이동

명령: "지금 main에 작업한 프로젝트 examples 폴더로 옮겨줘". 대상: 루트 `main/` 의 HA 제어 클라이언트 펌웨어(68e5428, `project(home_assistant_client)`). 방식: 2026-07-16 hotplate 이동(4a2d802)과 동일한 **진짜 이동(git mv)**. 사용자 확인 결과 — 폴더명 `home_assistant_client`, 루트 빌드 파일 3개 제거, 루트 README 는 예제로 이전 후 인덱스화. (see LP §2.1, §3.11, §5.7, §5.13)

- [x] `examples/home_assistant_client/main/` 생성 후 루트 `main/` 추적 파일 전체를 `git mv` — 실제 **12개**(ToDo 초안의 11개는 오기, hotplate 이동 때와 같은 실수): main.c, ui.c/h, network.c/h, buttons_check.c/h, ha_client.c/h, CMakeLists.txt, Kconfig.projbuild, idf_component.yml. 빈 `main/` 은 rmdir
- [x] `examples/home_assistant_client/CMakeLists.txt` — 새로 쓰지 않고 루트 `CMakeLists.txt` 를 `git mv`. 이미 `project(home_assistant_client)` 라 내용이 그대로 맞고, 히스토리도 보존됨 (hotplate 때는 신규 작성했음)
- [x] 루트 `sdkconfig.defaults` · `dependencies.lock` 을 `git mv` 로 예제 폴더에 이동 — 루트 defaults 는 이미 LVGL 8 죽은 심볼이 제거된 상태(68e5428)라 추가 수정 불필요. 빌드 로그 `unknown kconfig symbol` **0건**으로 검증
- [x] 미추적 `sdkconfig`(WiFi SSID·비밀번호·HA 장기 토큰 보유)를 예제 폴더로 물리 이동 — 새 위치에서도 `.gitignore:6` 의 `sdkconfig` 패턴에 걸리는 것을 `git check-ignore -v` 로 확인. menuconfig 재입력 없이 빌드 재현됨 (LP §2.1)
- [x] 사용자 결정: 루트 빌드 파일 3개 제거 — 위 `git mv` 3건이 곧 제거(루트에서 사라짐 + 히스토리 보존)라 별도 `git rm` 불필요. 루트 `managed_components/` 는 "historical reasons" 로 추적 유지 (.gitignore 주석)
- [x] 사용자 결정: 루트 `README.md` 309줄의 HA 상세 가이드를 `examples/home_assistant_client/README.md` (233줄)로 이전 — 상대경로 전부 `../../` 기준으로 교정, 링크 대상 10개 존재 확인. 이전 과정에서 stale 항목 정리: "Hardware required" 에 남아있던 Beszel 서버·claude_usage_server·RGB LED 요구사항(server_monitor 것)을 HA 기준으로 교체
- [x] 루트 `README.md` 를 6개 예제 인덱스로 재작성 — 프로젝트 선택 표, 1회성 호스트 셋업(VSCode·ESP-IDF 확장·Zadig), 공통 빌드/플래시, 저장소 레이아웃(루트에 `main/` 없음 반영), 프로젝트 공통 pitfall 10건, 신규 프로젝트 시작 절차. HA 전용 pitfall 4건은 예제 README 로 분리
- [x] `examples/README.md` 표에 `home_assistant_client` 행 추가 — 점검 중 `smart_plug` / `sy01b_firmware` 2개가 표에서 누락된 것을 발견해 함께 채움(총 6행). 본문의 "루트 프로젝트 없음" 문구는 4a2d802 에서 이미 정정돼 있어 수정 불필요
- [x] 새 위치에서 무경고 빌드 검증 (LP §5.7 env-dump 레시피, `run_in_background`) — exit 0, `[1897/1897] Project build complete`, **컴파일 경고 0건** + `unknown kconfig symbol` **0건**. `home_assistant_client.bin` = `0x139de0` (16% free) → 이동 전 루트 빌드 산출물과 **byte 단위로 동일**, 이동이 펌웨어를 바꾸지 않았음을 확인. LP §5.13 대로 사전에 잔여 빌드 프로세스 0개 확인 후 단일 빌드만 실행
- [x] 빌드 중 신규 경고 1건 분석 — mbedtls `CMAKE_OBJECT_PATH_MAX` (오브젝트 경로 254자 > 권고 250자). 예제 폴더가 루트보다 31자 깊어져 발생한 것으로, 우리 코드와 무관하며 `.obj` 정상 생성 + bin 크기 동일로 무해 확인. LearnedPatterns §5.15 에 기록
- [x] 이동 뒷정리 — 루트 `build/`(208MB, 사라진 `main/` 대상) 삭제. 삭제 전에 `.vscode/settings.json` 의 `clangd --compile-commands-dir` 을 새 빌드 경로로 갱신해 IntelliSense 유지
- [x] GitHub Issue [#24](https://github.com/coport-uni/ESP32S3WebMonitor/issues/24) 생성 + 커밋/푸시 — 소스 15개(main 12 + 루트 3) 전부 100% rename, `git diff --cached --stat -M` 기준 **0 insertions, 0 deletions**

메모(커밋 범위): `.vscode/settings.json` 은 커밋에서 제외했다. 이번 이동으로 필요해진 `clangd --compile-commands-dir` 경로 갱신 외에, 세션 시작 시점부터 사용자의 미커밋 변경(`idf.portWin` COM3→COM14, `idf.flashType` JTAG→UART)이 같은 파일에 들어 있어 함께 쓸려 들어가기 때문. 워킹트리에는 두 변경 모두 남아 있으므로 사용자가 원할 때 커밋하면 된다. `examples/server_monitor/` 의 미커밋 WIP(beszel/claude_usage/usage_led)과 미추적 `examples/smartplugcontroller_software/`, `.claude/` 로그류도 계획대로 제외.

메모(플래시 경로 변경): 다음 플래시부터는 VSCode 에서 저장소 루트가 아니라 `examples/home_assistant_client/` 를 열어야 한다. 루트에 `CMakeLists.txt` 가 더 이상 없어 ESP-IDF 확장이 프로젝트를 인식하지 못한다. 터미널에서는 해당 폴더로 `cd` 후 `idf.py -p COM14 flash monitor`. 이 안내는 루트 README 의 "One-time host setup > 4. Get the source" 에도 인용문으로 넣어 두었다.
