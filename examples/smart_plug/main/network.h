 ...
 * station mode using CONFIG_SMART_PLUG_WIFI_SSID /
 * CONFIG_SMART_PLUG_WIFI_PASSWORD, then keeps reconnecting on disconnect.
 * Returns immediately — the caller does not wait for association.
 *
 * If CONFIG_SMART_PLUG_WIFI_SSID is empty, this logs a warning and
 * returns ESP_OK without starting WiFi.
 */
 ...
 *
 * Safe to call from any task; reads a single word updated by the WiFi
 * event handler.
 *
