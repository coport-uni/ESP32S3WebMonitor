...
/* ----------------------------------------------------------------- Prime */
/* LVGL v9 dropped the v8 "active button text" idiom on msgbox. Each footer
 * button now fires its own LV_EVENT_CLICKED, so per-button callbacks carry
 * the modal pointer via user_data. */
static void prime_start_cb(lv_event_t *e)
{
...
static void prime_cancel_cb(lv_event_t *e)
{
    lv_obj_t *modal = (lv_obj_t *)lv_event_get_user_data(e);
    lv_msgbox_close(modal);
}
...
static void prime_btn_event_cb(lv_event_t *e)
{
    ...
    lv_msgbox_add_text(modal,
                       "Run one prime cycle:\n"
                       "port 3 (source) → port 1 (sink)\n"
                       "Full-stroke aspirate + dispense.");
    lv_obj_t *start_btn = lv_msgbox_add_footer_button(modal, "Start");
    lv_obj_t *cancel_btn = lv_msgbox_add_footer_button(modal, "Cancel");
    lv_obj_add_event_cb(start_btn, prime_start_cb, LV_EVENT_CLICKED, modal);
    lv_obj_add_event_cb(cancel_btn, prime_cancel_cb, LV_EVENT_CLICKED, modal);
    lv_obj_center(modal);
}
...
/* ----------------------------------------------------------------- Error modal
 *
 * LVGL v9: each footer button is its own widget firing LV_EVENT_CLICKED.
 * Per-button callbacks below replace the v8 LV_EVENT_VALUE_CHANGED idiom
 * that switched on the active button's label text.
 */
static void modal_close(void)
{
    if (s_modal != NULL) {
        lv_msgbox_close(s_modal);
        s_modal = NULL;
    }
}

static void modal_retry_cb(lv_event_t *e)
{
    (void)e;
...
static void modal_reinit_cb(lv_event_t *e)
{
    (void)e;
    pump_cmd_t cmd = {
...
void ui_show_error_modal(const pump_error_t *err)
{
    ...
    if (err->klass == PUMP_ERROR_FATAL) {
        lv_obj_t *btn = lv_msgbox_add_footer_button(s_modal, "Re-initialize");
        lv_obj_add_event_cb(btn, modal_reinit_cb, LV_EVENT_CLICKED, NULL);
    } else {
        lv_obj_t *retry_btn = lv_msgbox_add_footer_button(s_modal, "Retry");
        lv_obj_add_event_cb(retry_btn, modal_retry_cb, LV_EVENT_CLICKED, NULL);
        lv_obj_t *dismiss_btn =
            lv_msgbox_add_footer_button(s_modal, "Dismiss");
        lv_obj_add_event_cb(dismiss_btn, modal_dismiss_cb, LV_EVENT_CLICKED,
                            NULL);
    }
