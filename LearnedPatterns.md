...
### 5.15 Moving a project into `examples/` pushes mbedtls object paths past CMake's 250-char limit on Windows

- **Problem**: Building the Home Assistant client from its new home under `examples/home_assistant_client/` emitted a `CMake Warning` that had never appeared at the repository root: the object directory for `tf-psa-crypto/core` "has 179 characters. The maximum full path to an object file is 250 characters (see CMAKE_OBJECT_PATH_MAX)... The build may not work correctly." It names a stock ESP-IDF component, so it reads like a broken toolchain rather than a consequence of the move.
- **Cause**: `examples/home_assistant_client/` is 31 characters deeper than the repository root, and mbedtls nests its objects unusually deep (`build/esp-idf/mbedtls/mbedtls/tf-psa-crypto/core/CMakeFiles/tfpsacrypto.dir/<32-char hash>/`). Adding the 79-char object name puts the full path at 254 characters, over CMake's conservative 250-char guard, which exists because the classic Win32 API caps paths at `MAX_PATH` 260. The workspace already sits at `C:\Users\<20-char user>\Desktop\workspace\container\Espress_dev`, so the headroom was thin before the move.
- **Fix**: Nothing — the warning is advisory. The build completed (exit 0, `[1897/1897]`, zero compiler warnings) and the 254-char `psa_crypto_driver_esp_hmac_transparent.c.obj` was written normally, because this host has Windows long-path support. The verification that mattered was the output, not the warning: `home_assistant_client.bin` came out `0x139de0` bytes, byte-for-byte the size of the pre-move root build.
- **Rule**: When a project moves deeper in the tree, expect a `CMAKE_OBJECT_PATH_MAX` warning from mbedtls and check the *artifact* rather than the warning — a produced `.obj` and an unchanged binary size mean it is cosmetic. Never treat it as a reason to skip the move. It does bound where this repo can be cloned: a checkout more than ~40 characters deeper than `C:\Users\<user>\Desktop\workspace\container\` may genuinely fail to build, so clone near the drive root (e.g. `C:\esp\`) on hosts without long-path support. (from ToDo: 2026-08-20 Home Assistant 클라이언트를 examples/home_assistant_client 로 이동)
